...
def pass_money (bank, money):
    bank.money = money

def collect_money(banks):
    job = [gevent.spawn(get_data_from_url, bank) for bank in banks]
    gevent.joinall(job)
    datas = [thread.value for thread in job if thread.value != Money("0", "0")]
    map(pass_money, banks, datas)
    return banks
